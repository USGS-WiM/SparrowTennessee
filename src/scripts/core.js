...
require([
   ...
   //get chartOutfields from config --i.e {attribute: "VALUE", label: "value"}
    ...
    $("chartModal").on('shown.bs.modal', function(event){
     ...
     function highlightMapFeature(category){
        var queryTask;
        queryTask = new esri.tasks.QueryTask("https://gis.wim.usgs.gov/arcgis/rest/services/SparrowTennessee/SparrowTennesseeTest/MapServer/0");
        var query = new esri.tasks.Query();
        query.returnGeometry = true;
        query.outFields = "GRP_3_NAM";
        query.where = "GRP_3_NAM = '" + category + "'";

        queryTask.execute(query, handleHighlight);
    }

    function handleHighlight(response){
        console.log("in highlight() response function");
    }
